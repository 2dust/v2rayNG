...
/**
 * ComposeToast — 参考 2dust/v2rayNG 的 Toasty 风格重构
 *
 * 修复项：
 * 1. 底部偏移量增大到 120dp，确保不被 FAB 遮挡
 * 2. 背景色改为半透明（alpha ≈ D8/E0），实现毛玻璃/透明效果
 * 3. 保持 normal / success / error / info 四种类型
 * 4. 带有左侧圆形图标指示（✓ ✕ ℹ 等）
 * 5. 深色/浅色模式自适应
 * 6. 圆角胶囊形背景
 * 7. 线程安全，可在任意线程调用
...
object ComposeToast {
    ...
    private const val BG_NORMAL_LIGHT  = 0xB3353A3E.toInt()  // 半透明深灰
    private const val BG_NORMAL_DARK   = 0xB34A4F54.toInt()  // 半透明中灰
    private const val BG_SUCCESS_LIGHT = 0xB3388E3C.toInt()  // 半透明绿色
    private const val BG_SUCCESS_DARK  = 0xB3388E3C.toInt()
    private const val BG_ERROR_LIGHT   = 0xB3D50000.toInt()  // 半透明红色
    private const val BG_ERROR_DARK    = 0xB3D50000.toInt()
    private const val BG_INFO_LIGHT    = 0xB33F51B5.toInt()  // 半透明蓝色
    private const val BG_INFO_DARK     = 0xB33F51B5.toInt()

    private const val TEXT_COLOR       = 0xFFFFFFFF.toInt()  // 白色文字
    private const val ICON_COLOR       = 0xFFFFFFFF.toInt()  // 白色图标
    ...
    private fun getIconText(type: ToastType): String? = when (type) {
        ...
        ToastType.NORMAL  -> null  // normal 类型无图标
    }
    ...
    /**
     * 自定义圆形图标 View（参考 Toasty 的左侧圆形图标）
     * 在圆形背景上绘制符号文字
     */
    ...
    private fun showStyledToast(
        ...
        val action = Runnable {
            try {
                ...
                val cornerRadius = dpToPx(appContext, 24f)  // 胶囊形圆角
                ...
                // 修复：增大底部偏移，确保 Toast 显示在 FAB 按钮之上
                toast.setGravity(
