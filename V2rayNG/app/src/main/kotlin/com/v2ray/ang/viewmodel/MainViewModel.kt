...
class MainViewModel(application: Application) : AndroidViewModel(application)
{
    ...
    var subscriptionId: String =
        settingsStorage.decodeString(AppConfig.CACHE_SUBSCRIPTION_ID, "") ?: ""
    var keywordFilter: String =
        settingsStorage.decodeString(AppConfig.CACHE_KEYWORD_FILTER, "") ?: ""
        private set
...
//        builder.show()
    ...
    fun removeDuplicateServer()
    {
        ...
        serversCache.forEach { it ->
            it.config.getProxyOutbound()?.let { outbound ->
                serversCache.forEach { it2 ->
                    if (it2.guid != it.guid && outbound == it2.config.getProxyOutbound() && !seen.contains(
                            it2.guid
                        )
                    )
                    {
